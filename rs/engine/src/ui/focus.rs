...
use crate::todo_warn;
use crate::ui::{GLOBAL_UI_RENDER_LAYERS, GlobalUi, MenuStack, UiAction, UiCam};
use crate::ui::widgets::{WidgetGizmos, WidgetShape};
...
impl FocusTarget {
	pub fn resolve(&self, from: Entity, parent: Option<&Parent>, children_query: &Query<&Children>) -> Option<Entity> {
		match self {
			FocusTarget::NextSibling => parent
				.map(Parent::get)
				.and_then(|parent| children_query.get(parent).ok())
				.and_then(|children| children.get((children.iter().position(|child| *child == from).unwrap() + 1) % children.len()).copied()),
			FocusTarget::PrevSibling => parent
				.map(Parent::get)
				.and_then(|parent| children_query.get(parent).ok())
				.and_then(|children| children.get((children.iter().position(|child| *child == from).unwrap() + children.len() - 1) % children.len()).copied()),
			FocusTarget::Parent => parent.map(Parent::get),
			FocusTarget::Child(i) => children_query.get(from).ok()
...
pub fn resolve_focus(
	...
	for (state, layers) in std::iter::once((&*glob_actions, &GLOBAL_UI_RENDER_LAYERS)).chain(actions.iter()) {
		let Some((mut cam, _)) = cams.iter_mut()
			.find(|(_, cam_layers)| **cam_layers == *layers)
		else {
			error!("`UiCam` should exist for all `RenderLayers` for which `ActionState<UiAction>` exists");
			continue
		};
		let Some((curr_id, mut curr)) = cam.focus.and_then(|curr_id| {
			q.get(curr_id).ok().map(|curr| (curr_id, curr))
		}) else { continue };
		...
		if let Some(dir) = state.clamped_axis_pair(&UiAction::MoveCursor) {
			if dir.length() > 0.5 {
				let (dir, target) = curr.0.directions.iter().find_map(|(wedge, target)| {
...
pub fn highlight_focus<const LAYER: Layer>(
	...
	let Some(focus) = cams.iter()
		.find(|(cam, cam_layers)| **cam_layers == RenderLayers::layer(LAYER))
		.and_then(|(cam, _)| cam.focus) else { return };
	match q.get(focus) {
